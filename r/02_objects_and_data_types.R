...
test_vect <- test_vect[-2]  # removing an element (in this case the second element)
...
matrix(c(1, 2, 3, 4, 5, 6), nrow = 3, ncol = 2, byrow = TRUE)  # You can ask it to fill-in row-wise


dim(test_mat)   # check the dimensions of the matrix (OWS first and COLUMNS second)
...
matrix(1:5, nrow = 3, ncol = 3)  # it doenst work, because the length of the vector doesn't match the dimensions
...
test_mat[1,2] # on two dimensional objects, we call ROWS first and COLUMNS second. e.g. [ROW, COLUMN] - Like playing battleship!
...
test_mat["row_1",]  # You still need to add the comma and the empty space for the other coordinate
test_mat[,"col_a"]  # You still need to add the comma and the empty space for the other coordinate




### NOTE: Matrices are used quite a bit for multivariate statistics (e.g. PCA). At least for now. 
...
# the most useful. Similar to named matrices, but (!) allow for different types of data in the same objects
...
test_df <- data.frame(var_1 = c(1, 2, 3, 4, 5, 6),
                      ...
                      var_3 = c("a", "b", "c", "d", "e", "f"),
                      var_3 = c(TRUE, FALSE, FALSE, TRUE, TRUE, FALSE))  # you can specify the variable names right here!!

test_df

dim(test_df)  # you can also check the dimmesnions of your df
...
# in a data frame, columns (variables) and rows are named from the begining

colnames(test_df)

rownames(test_df)   # row names are only marginally useful, and you are better off creating a column of IDs if you really want to keep and use that info
...
test_df[,1]          # Equivalent to what we did before in matrix, but much easier to call things by their name (as long as you kept it short and neat!)
test_df[,"var_1"]    # The "$var_1" approach is simpler 

# I do recommend the "$var_1" approach. Among other reasons, because it simplifies getting specific objects
...
### NOTE: Tibbles, the Tidyverse version of a dataframe exist and are (generally) better.
...
# Knowing how to "dig" through them, will help you make better use of these outputs
...
test_list[1]   # Need the [[]]. If you call it wit [] it access the first element, but the new object is still a list, harder to handle
